...
def search_products(keyword: str, category: str = None) -> Dict[str, Any]:
    """
    搜尋商品工具
    
    根據關鍵字與分類搜尋商品庫存中符合的項目。
    """
    time.sleep(0.1)
    
    results = []
    # 遍歷 PRODUCTS 資料庫進行簡單搜尋
    for sku, info in PRODUCTS.items():
        # 檢查關鍵字是否在名稱中 (不分大小寫)
        name_match = keyword.lower() in info["name"].lower()
        
        if name_match:
            results.append({
                "sku": sku,
                "name": info["name"],
                "in_stock_qty": info["in_stock_qty"]
            })
    ...
    return {
        ...
        "results": results,
        "count": len(results),
        "message": f"搜尋 '{keyword}' 找到 {len(results)} 筆結果。"
    }
